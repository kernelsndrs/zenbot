...
module.exports = function container (get, set, clear) {
  ...
  return function (s) {
    ...
    if (!s.product) {
      s.error_log.log('error: could not find product "' + s.product_id + '"')
      process.exit(1)
    }
    ...
    function createTextUI() {
      ...
      s.error_log = s.grid.set(3, 0, 2, 12, blessed.log, {
        label: 'Errors',
        style: {
          focus: {
            border: {
      ...
      s.info_log = s.grid.set(5, 0, 3, 12, blessed.log, {
        label: 'Info',
        style: {
          focus: {
            border: {
      ...
      s.recent_periods = s.grid.set(8, 0, 3, 12, blessed.log, {
      ...
      s.current_data = s.grid.set(11, 0, 2, 12, blessed.log, {
        label: 'Current Data',
        style: {
          focus: {
            border: {
      ...
      s.current_data.focus()
      if(c.TUI.line_chart) {
        s.line_chart = s.grid.set(0, 0, 3, 6, contrib.line, {
          label: 'Market Data',
          style: {
            focus: {
              fg: 'blue'
        ...
        s.screen.on('resize', function() {
          s.line_chart.emit('attach')
        })

      }

      s.screen.render()
      s.screen.key(['right'], function(ch, key) {
        s.screen.focusNext()
      })
      s.screen.key(['left'], function(ch, key) {
        s.screen.focusPrevious()
      })
    ...
    function updateLineData() {
      if(s.line_chart_data.x.length > 7){
        s.line_chart_data.x.shift()
      }
      if(s.line_chart_data.y.length > 7){
        s.line_chart_data.y.shift()
      }
      s.line_chart_data.x.push(moment().format('HH:mm:ss'))
      s.line_chart_data.y.push(s.period.close)
      s.line_chart.setData([s.line_chart_data])
      s.screen.render()
    }
    ...
    function msg (str) {
      if (so.debug) {
        s.info_log.log(moment().format('YYYY-MM-DD HH:mm:ss') + ' - ' + str)
      }
    }
        ...
        if (!s.acted_on_stop) {
          if (last_trade.type === 'buy') {
            if (do_sell_stop && s.sell_stop && s.period.close < s.sell_stop) {
              stop_signal = 'sell'
              s.info_log.log(('sell stop triggered at ' + pct(s.last_trade_worth) + ' trade worth').red)
            }
            ...
            if (s.profit_stop && s.period.close < s.profit_stop && s.last_trade_worth > 0) {
              stop_signal = 'sell'
              s.info_log.log(('profit stop triggered at ' + pct(s.last_trade_worth) + ' trade worth').green)
            }
          }
          else {
            if (s.buy_stop && s.period.close > s.buy_stop) {
              stop_signal = 'buy'
              s.info_log.log(('buy stop triggered at ' + pct(s.last_trade_worth) + ' trade worth').red)
            }
          }
        }
      }
      ...
      let cb = function (err, order) {
        ...
        if (err) {
          if (_cb) {
            ...
            s.error_log.log((err.message + ': ' + err.desc).red)
          } else {
            memDump()
            s.error_log.log(err)
          }
            ...
            if (!size) {
              if (so.mode === 'live' || so.mode === 'paper') {
                ...
                if(so.buy_max_amt){ // account for held assets as buy_max
                  ...
                  buy_pct = n(adjusted_buy_max_amt).divide(s.balance.currency).multiply(100).value()
                }else{ // account for held assets as %
                  let held_pct = n(s.asset_capital).divide(s.balance.currency).multiply(100).value()
                  buy_pct = n(so.buy_pct).subtract(held_pct).value()
                }
    ...
    function executeOrder (trade) {
      ...
      if (s.buy_order) {
        if (so.mode === 'live' || trade.price <= Number(s.buy_order.price)) {
          ...
          if (so.stats) {
            s.info_log.log('buy'.bgGreen + (' order completed at ' + moment(trade.time).format('YYYY-MM-DD HH:mm:ss') + ':').green)
            s.info_log.log((fa(my_trade.size) + ' at ' + fc(my_trade.price)).green)
            s.info_log.log(('total ' + fc(my_trade.size * my_trade.price)).green)
            s.info_log.log((n(my_trade.slippage).format('0.0000%') + ' slippage (orig. price ' + fc(s.buy_order.orig_price) + ')').green)
            s.info_log.log(('execution: ' + moment.duration(my_trade.execution_time).humanize()).green)
            order_complete = 'buy order completed at ' + moment(trade.time).format('YYYY-MM-DD HH:mm:ss') + ':\n\n' + fa(my_trade.size) + ' at ' + fc(my_trade.price) + '\ntotal ' + fc(my_trade.size * my_trade.price) + '\n' + n(my_trade.slippage).format('0.0000%') + ' slippage (orig. price ' + fc(s.buy_order.orig_price) + ')\nexecution: ' + moment.duration(my_trade.execution_time).humanize() + '\n'
        ...
        if (so.mode === 'live' || trade.price >= s.sell_order.price) {
          ...
          s.my_trades.push(my_trade)
          if (so.stats) {
            s.info_log.log('sell'.red + (' order completed at ' + moment(trade.time).format('YYYY-MM-DD HH:mm:ss') + ':').green)
            s.info_log.log((fa(my_trade.size) + ' at ' + fc(my_trade.price)).green)
            s.info_log.log(('total ' + fc(my_trade.size * my_trade.price)).green)
            s.info_log.log((n(my_trade.slippage).format('0.0000%') + ' slippage (orig. price ' + fc(s.sell_order.orig_price) + ')').green)
            s.info_log.log(('execution: ' + moment.duration(my_trade.execution_time).humanize()).green)
            order_complete = 'sell order completed at ' + moment(trade.time).format('YYYY-MM-DD HH:mm:ss') + ':\n\n' + fa(my_trade.size) + ' at ' + fc(my_trade.price) + '\ntotal ' + fc(my_trade.size * my_trade.price) + '\n' + n(my_trade.slippage).format('0.0000%') + ' slippage (orig. price ' + fc(s.sell_order.orig_price) + ')\nexecution: ' + moment.duration(my_trade.execution_time).humanize() + '\n'
    ...
    function writeReport (is_period, blink_off) {

      let reportLine = ''
      reportLine += (moment().format('YYYY-MM-DD HH:mm:ss')[is_period ? 'bgBlue' : 'white'])
      ...
      reportLine += (volume_display.cyan)
      ...
      if(!is_period){
        s.current_data.insertBottom(reportLine)
        s.screen.render()
      }else {
        //report Period
        if(typeof s.line_chart_data!== 'undefined') updateLineData()
        s.recent_periods.insertBottom(reportLine)
        s.screen.render()

      }
    }

    function update (trades, is_preroll, cb) {
